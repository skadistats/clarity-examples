# examples-replay-chooser Specification

## Purpose

Defines how examples obtain the replay file for their invocation: a single shared component (ReplayChooser) with a fixed args → env → history → native OS file picker cascade, using JDK Preferences for a single global last-chosen replay shared across every example. Callers receive the absolute path and continue to own MappedFileSource lifecycle themselves.
