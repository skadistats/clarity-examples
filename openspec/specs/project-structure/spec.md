...
Each example directory SHALL produce a `<name>Run` task (runs `Main` with optional `--args`) and a `<name>Package` task (emits a self-contained fat jar under `build/libs/`). These tasks SHALL be generated by a shared convention plugin in `build-logic/`, not duplicated across subproject `build.gradle.kts` files.
...
#### Scenario: Fat-jar packaging is preserved
