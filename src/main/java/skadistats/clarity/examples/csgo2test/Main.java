package skadistats.clarity.examples.csgo2test;
...
public class Main {
    ...
    private static void runDump(Replay... replays) throws Exception {
        for (Replay replay : replays) {
            ...
            new skadistats.clarity.examples.dump.Main().run(replay.getFile(), false, false);
        }
    }
    ...
    private static void runEntities(Replay... replays) throws Exception {
        for (Replay replay : replays) {
            ...
            new skadistats.clarity.examples.entityrun.Main().run(new String[]{replay.getFile()});
        }
    }
    ...
    private static void runStringTables(Replay... replays) throws Exception {
        for (Replay replay : replays) {
            ...
            new skadistats.clarity.examples.stringtabledump.Main().runSeek(new String[]{replay.getFile()});
        }
    }
