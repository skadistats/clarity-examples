...
public @interface OnAbilityCooldownReset {

    interface Listener {
        void invoke(Entity ability, Entity owner);
    }

    final class Event extends skadistats.clarity.event.Event<OnAbilityCooldownReset> {
        private final Listener[] typedListeners;

        public Event(Runner runner, Class<OnAbilityCooldownReset> eventType, Set<EventListener<OnAbilityCooldownReset>> listeners) {
